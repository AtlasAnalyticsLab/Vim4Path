...
def compute_from_patches(args, wsi_object, clam_pred=None, model=None, feature_extractor=None, batch_size=512,
    ...
    for idx, (roi, coords) in enumerate(roi_loader):
        ...
        with torch.no_grad():
            if "vit" in args['arch']:
                intermediate_output = feature_extractor.get_intermediate_layers(roi, args['n_last_blocks'])
                features = torch.cat([x[:, 0] for x in intermediate_output], dim=-1)
                if args['avgpool_patchtokens']:
                    features = torch.cat((features.unsqueeze(-1),
            ...
            if attn_save_path is not None and False:
