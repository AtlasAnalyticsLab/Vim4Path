...
def get_feature_extractor():
	...
	model.cuda()
	model.eval()
	args['pretrained_weights'] = '/home/ubuntu/checkpoints/camelyon16_224_5x/vim-s_224-96_2/checkpoint.pth'
...
if __name__ == '__main__':
	...
	for i in range(len(process_stack)):
		...
		if not os.path.isfile(h5_path) :
			_, _, wsi_object = compute_from_patches(args=args, wsi_object=wsi_object,
											model=model, 
		...
		if heatmap_args.calc_heatmap:
			compute_from_patches(args=args, wsi_object=wsi_object, clam_pred=Y_hats[0], model=model, feature_extractor=feature_extractor, batch_size=exp_args.batch_size, **wsi_kwargs,
